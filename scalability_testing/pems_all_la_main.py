...
def train(train_dataloader, val_dataloader, mean, std, epochs, seq_length, num_nodes, num_features, allGPU=False, debug=False):
    ...
    sensor_ids, sensor_id_to_ind, adj_mx = load_graph_data("data/all_la_adj.pkl")
...
def main():
    ...
    if args.mode == "base":
        # preprocess data
        pre_t1 = time.time()
        x_tensor,y_tensor = benchmark_preprocess("data/speed2GB.h5","pems-all-la")
        ...
        if allGPU == True:
            data, mean, std, x_train, x_val, x_test = IndexDataset.preprocess("data/speed2GB.h5", 12,"pems-all-la", h5_key="df", add_time_in_day=True,gpu=0)
        else:
            data, mean, std, x_train, x_val, x_test = IndexDataset.preprocess("data/speed2GB.h5",12,"pems-all-la", h5_key="df",add_time_in_day=True)
        # print(mean,std)
        # exit()
        pre_t2 = time.time()
        print("Preprocessing: ", pre_t2 - pre_t1, flush=True)
