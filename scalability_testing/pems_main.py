...
def main():
    ...
    if args.mode == "base":
        # preprocess data
        pre_t1 = time.time()
        x_tensor,y_tensor = benchmark_preprocess("data/speed.h5","pems")
        ...
        if allGPU == True:
            data, mean, std, x_train, x_val, x_test = IndexDataset.preprocess("data/speed.h5", 12,"pems", h5_key="df", add_time_in_day=True,gpu=0)
        else:
            data, mean, std, x_train, x_val, x_test = IndexDataset.preprocess("data/speed.h5",12,"pems", h5_key="df",add_time_in_day=True)
        # print(mean,std)
        # exit()
        pre_t2 = time.time()
        print("Preprocessing: ", pre_t2 - pre_t1, flush=True)
